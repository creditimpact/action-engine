...
async def test_register_engine_and_allowed_action():
    ...
    result = await main.actions_check_endpoint(
...
async def test_missing_headers_return_403():
    with pytest.raises(HTTPException) as exc:
        await main.actions_check_endpoint({"engine_id": "a", "platform": "gmail", "action_type": "send"}, x_engine_id=None, x_engine_key=None)
    assert exc.value.status_code == 403

@pytest.mark.asyncio
async def test_invalid_headers_return_403():
    with pytest.raises(HTTPException) as exc:
        await main.actions_check_endpoint(
            {"engine_id": "a", "platform": "gmail", "action_type": "send"},
            x_engine_id="bad",
            x_engine_key="bad",
        )
